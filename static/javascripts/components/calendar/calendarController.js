...
app.controller("calendarCtrl", function($scope, $filter, $q, $timeout, $log,
    ...
    $scope.prevMonth = function(data) {
        $scope.msg = "You clicked (prev) month " + data.month +
            ", " + data.year;
    };

    $scope.nextMonth = function(data) {
        $scope.msg = "You clicked (next) month " + data.month +
            ", " + data.year;
    };

    $scope.setContentViaService = function() {
        var today = new Date();
        MaterialCalendarData.setDayContent(today,
            '<span> :oD </span>')
    };
    
    /* change this out this is not required anymore */
    var holidays = {
        "2015-01-01": [{
    ...
    $scope.setDayContent = function(date) {
        ...
        var key = [date.getFullYear(), numFmt(date.getMonth() + 1),
            numFmt(date.getDate())
        ].join("-");
        var data = (holidays[key] || [{
            name: ""
        }])[0].name;
        if (loadContentAsync) {
